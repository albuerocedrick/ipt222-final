...
<div :class=" [ 'h-20 bg-base-100', 'box', 'flex', 'flex-row', 'items-center', 'justify-between']">
        ...
        <h1 v-else-if="$route.path === '/main/projects'" :class="['font-bold', 'lg:text-2xl md:text-xl text-sm', 'm-8', 'text-base-content']">  Projects</h1>
        <h1 v-else-if="$route.path === '/main/settings'" :class="['font-bold', 'lg:text-2xl md:text-xl text-sm', 'm-8', 'text-base-content']">  Settings</h1>
        ...
        <div v-else class="flex flex-row gap-2 items-center">
            ...
            <dialog id="my_modal_3" class="modal">
                <div class="modal-box box">
                    <div>
                        <h1 class="font-bold text-xl mb-4">Create a Task</h1>
                        <form method="dialog">
                            <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2 ">✕</button>
                        </form>
                    </div>    
                    <form class="flex flex-col items-center">
                        <div class="flex flex-row gap-2 items-center pt-4 border-t  border-neutral/30 w-full">
                            ...
                            <input type="text" placeholder="Name of Task" class="input rounded-2xl border-0 focus:outline-0 h-10 font-medium bg-base-200 flex-1 " />
                        </div>
